...
abstract class BossBarGame(world: World, bukkitPlayers: List<Player>, difficulty: GameDifficulty = GameDifficulty.NORMAL) : Game(world, bukkitPlayers, difficulty) {
    lateinit var bossBar: SimpleBossBar
        private set

    open val bossBarCreator: () -> SimpleBossBar = { SimpleBossBar(target(false), 20, { component("Turn Countdown: ${turnCountdown}s") }, { (turnCountdown.toFloat() / (info.turnTimer * difficulty.timeFactor).toInt()).coerceIn(0.0f, 1.0f) }, { BossBar.Color.YELLOW }, { BossBar.Overlay.NOTCHED_10 }) }
