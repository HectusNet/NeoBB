...
class TurnEvents: Listener {
    ...
    private fun handleStructure(event: BlockPlaceEvent, block: Block, player: NeoPlayer) {
        ...
        if (player.game is HectusGame) {
            when (structure.name) {
                "iron_bar_jail" -> player.game.turn(TIronBarJail(placed, cord, player), event)
                "oak_door_turtling" -> player.game.turn(TOakDoorTurtling(placed, cord, player), event)
                "glass_wall" -> player.game.turn(TGlassWall(placed, cord, player), event)
                "blue_glass_wall" -> player.game.turn(TBlueGlassWall(placed, cord, player), event)
                "orange_glass_wall" -> player.game.turn(TOrangeGlassWall(placed, cord, player), event)
                "pink_glass_wall" -> player.game.turn(TPinkGlassWall(placed, cord, player), event)
                "red_glass_wall" -> player.game.turn(TRedGlassWall(placed, cord, player), event)
            }
        }
        if (player.game is DefaultGame) {
            when (structure.name) {
                "daylight_sensor_line" -> player.game.turn(TDaylightSensorLine(placed, cord, player), event)
                "pumpkin_wall" -> player.game.turn(TPumpkinWall(placed, cord, player), event)
                "redstone_wall" -> player.game.turn(TRedstoneWall(placed, cord, player), event)
                "green_glass_wall" -> player.game.turn(TGreenGlassWall(placed, cord, player), event)
                "white_glass_wall" -> player.game.turn(TWhiteGlassWall(placed, cord, player), event)
                "amethyst-warp" -> player.game.turn(TAmethystWarp(placed, player), event)
                "cliff-warp" -> player.game.turn(TCliffWarp(placed, player), event)
                "desert-warp" -> player.game.turn(TDesertWarp(placed, player), event)
                "end-warp" -> player.game.turn(TEndWarp(placed, player), event)
                "frozen-warp" -> player.game.turn(TFrozenWarp(placed, player), event)
                "meadow-warp" -> player.game.turn(TMeadowWarp(placed, player), event)
                "mushroom-warp" -> player.game.turn(TMushroomWarp(placed, player), event)
                "nerd-warp" -> player.game.turn(TNerdWarp(placed, player), event)
                "nether-warp" -> player.game.turn(TNetherWarp(placed, player), event)
                "ocean-warp" -> player.game.turn(TOceanWarp(placed, player), event)
                "redstone-warp" -> player.game.turn(TRedstoneWarp(placed, player), event)
                "sun-warp" -> player.game.turn(TSunWarp(placed, player), event)
                "void-warp" -> player.game.turn(TVoidWarp(placed, player), event)
                "wood-warp" -> player.game.turn(TWoodWarp(placed, player), event)
            }
        }
        if (player.game is LegacyGame) {
            when (structure.name) {
                ...
                "white_glass_wall" -> player.game.turn(TWhiteGlassWall(placed, cord, player), event)
                "light_blue_glass_wall" -> player.game.turn(LTLightBlueGlassWall(placed, cord, player), event)
                "legacy-aether-warp" -> player.game.turn(LTAetherWarp(placed, player), event)
                "legacy-amethyst-warp" -> player.game.turn(LTAmethystWarp(placed, player), event)
                "legacy-book-warp" -> player.game.turn(LTBookWarp(placed, player), event)
                "legacy-cliff-warp" -> player.game.turn(LTCliffWarp(placed, player), event)
                "legacy-end-warp" -> player.game.turn(LTEndWarp(placed, player), event)
                "legacy-ice-warp" -> player.game.turn(LTIceWarp(placed, player), event)
                "legacy-mushroom-warp" -> player.game.turn(LTMushroomWarp(placed, player), event)
                "legacy-nether-warp" -> player.game.turn(LTNetherWarp(placed, player), event)
                "legacy-redstone-warp" -> player.game.turn(LTRedstoneWarp(placed, player), event)
                "legacy-snow-warp" -> player.game.turn(LTSnowWarp(placed, player), event)
                "legacy-sun-warp" -> player.game.turn(LTSunWarp(placed, player), event)
                "legacy-underwater-warp" -> player.game.turn(LTUnderwaterWarp(placed, player), event)
                "legacy-void-warp" -> player.game.turn(LTVoidWarp(placed, player), event)
                "legacy-wood-warp" -> player.game.turn(LTWoodWarp(placed, player), event)
                "legacy-heaven-warp" -> player.game.turn(if (Randomizer.boolByChance(55.0)) LTHeavenWarp(placed, player) else LTHellWarp(placed, player), event)
                "legacy-hell-warp" -> player.game.turn(if (Randomizer.boolByChance(55.0)) LTHellWarp(placed, player) else LTHeavenWarp(placed, player), event)
            }
        }
        if (player.game is PersonGame) {
            when (structure.name) {
                "candle_circle" -> player.game.turn(PTCandleCircle(placed, cord, player), event)
                "pumpkin_wall" -> player.game.turn(PTPumpkinWall(placed, cord, player), event)
                "stone_wall" -> player.game.turn(PTStoneWall(placed, cord, player), event)
                "torch_circle" -> player.game.turn(PTTorchCircle(placed, cord, player), event)
                "oak_door_turtling" -> player.game.turn(PTTurtling(placed, cord, player), event)
                "wood_wall" -> player.game.turn(PTWoodWall(placed, cord, player), event)
                "person-amethyst-warp" -> player.game.turn(PTAmethystWarp(placed, player), event)
                "person-fire-warp" -> player.game.turn(PTFireWarp(placed, player), event)
                "person-ice-warp" -> player.game.turn(PTIceWarp(placed, player), event)
                "person-snow-warp" -> player.game.turn(PTSnowWarp(placed, player), event)
                "person-villager-warp" -> player.game.turn(PTVillagerWarp(placed, player), event)
                "person-void-warp" -> player.game.turn(PTVoidWarp(placed, player), event)
            }
        }
    }
